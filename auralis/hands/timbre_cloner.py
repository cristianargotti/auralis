...
def analyze_timbre(
    ...
    # Find amplitude at each harmonic frequency
    harmonic_amps: list[float] = []
    fundamental_amp = 0.0

    for h in range(1, n_harmonics + 1):
        target_freq = median_f0 * h
        if target_freq > sr / 2:  # Nyquist
            break

        # Find nearest FFT bin
        bin_idx = int(round(target_freq / (sr / n_fft)))
        if bin_idx >= len(avg_spectrum):
            break

        # Search +/- 2 bins for the actual peak (slight detuning)
        search_lo = max(0, bin_idx - 2)
        search_hi = min(len(avg_spectrum), bin_idx + 3)
        peak_amp = float(np.max(avg_spectrum[search_lo:search_hi]))

        if h == 1:
            fundamental_amp = max(peak_amp, 1e-10)

        # Normalize relative to fundamental
        harmonic_amps.append(round(peak_amp / fundamental_amp, 4))
    ...
    # Simple approach: lowpass the spectrum to get noise envelope
...
def clone_render(
    ...
    # ── 1. Additive harmonics ──
    harmonic_signal = np.zeros(n_samples, dtype=np.float64)

    for h_idx, amp in enumerate(model.harmonic_amps):
        harmonic_num = h_idx + 1  # 1-based
        freq = target_freq * harmonic_num

        if freq >= sr / 2:  # Nyquist guard
            break

        # Each harmonic is a sine wave weighted by cloned amplitude
    ...
    if model.noise_profile and len(model.noise_profile) > 0:
        ...
        # Resample noise profile to match FFT length
        ...
        if len(profile) > 0:
            ...
            if len(profile) > 1:
                ...
                interpolator = interp1d(x_old, profile, kind="linear")
                noise_shape = interpolator(x_new)
            else:
                noise_shape = np.ones(target_len)

            # Normalize noise shape
    ...
    # ── 3. Mix harmonics + noise at cloned ratio ──
    ...
    if model.amp_envelope and len(model.amp_envelope) > 1:
        ...
        interpolator = interp1d(x_old, env, kind="linear")
